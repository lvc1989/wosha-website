            ...
            <Reveal key={cat} delay={ci * 80}>
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {services.filter((s) => (s.category || "Services") === cat).map((s) => (
                  <div key={s.id} className="flex items-center justify-between px-5 py-4 rounded-xl transition-colors duration-200 hover:bg-white/[0.07]" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
                    <div>
                    ...
                    <div className="font-display font-bold text-sm" style={{ color: "#FFDE00" }}>{money(s.price)}</div>
                  </div>
                ))}
              </div>
            </Reveal>

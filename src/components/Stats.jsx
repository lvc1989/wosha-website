...
export default function Stats({ branchCount, serviceCount, categoryCount, services = [] }) {
  const { t } = useLanguage();
  if (!branchCount && !serviceCount) return null;
  const durations = services.map((s) => Number(s.duration_min)).filter((n) => n > 0);
  ...
  return (
    <section className="py-14" style={{ background: "#0B1B33" }}>
      <div className="max-w-4xl mx-auto px-6 flex flex-wrap justify-center gap-y-8">
        ...
        {avgDuration != null && <Stat value={avgDuration} suffix="min" label={t("stats_turnaround")} />}
      </div>
    </section>
  );
}
